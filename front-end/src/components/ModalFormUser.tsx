...
export default function ModalFormUser({ isOpen, closeModal, user }: { isOpen: boolean, closeModal: () => void, user?: any }) {
    ...
    const handleCreate = async (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        try {
            const { full_name, email, password, confirm_password, role } = formData;
            setStatus({ isError: false, isLoading: true, message: "" });
            console.log(full_name, email, password, confirm_password, role);
            ...
            <div className="bg-white rounded-sm shadow-lg w-full max-w-md relative">
                <div className="w-full p-4">
                    <div className="w-full flex justify-between items-center">
                        <div>
                            <h1 className="text-2xl font-semibold capitalize">form user</h1>
